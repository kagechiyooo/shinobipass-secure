...
export function VerifyGesturesView({ selectedGestures, signatures, verifiedCount, onBack, onVerifyStep }: VerifyGesturesViewProps) {
  ...
  const handlePerformSign = () => {
    ...
    // Threshold for similarity (experimentally determined, around 0.1-0.2 is usually good)
    if (score < 0.25) {
      onVerifyStep();
      setSimilarity(null);
    } else {
      ...
      <div className="grid grid-cols-4 gap-3">
        {selectedGestures.map((id, index) => {
          ...
          return (
            <div
              key={id}
              className={`p-3 rounded-xl border-2 transition-all flex flex-col items-center space-y-1 ${isVerified
